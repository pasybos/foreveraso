...
def init_db():
    ...
    for col in ["panel_client_id", "uuid", "current_link", "ref_link"]:
        try:
            c.execute("ALTER TABLE users ADD COLUMN %s TEXT DEFAULT NULL" % col)
...
def get_user(tg_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT tariff, expire_time, last_free_request, used_free, ref_count, referrer_id, current_link, ref_link, panel_client_id, uuid FROM users WHERE tg_id=?", (tg_id,))
    row = c.fetchone()
    conn.close()
    return row


def add_or_update_user(tg_id, tariff, expire_ts, last_free=0, used_free=0, ref_count=0,
                       referrer_id=None, current_link=None, ref_link=None,
                       panel_client_id=None, uuid=None):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""INSERT OR REPLACE INTO users
        (tg_id, tariff, expire_time, last_free_request, used_free, ref_count,
         referrer_id, current_link, ref_link, panel_client_id, uuid)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (tg_id, tariff, expire_ts, last_free, used_free, ref_count,
         referrer_id, current_link, ref_link, panel_client_id, uuid))
    conn.commit()
    conn.close()
...
def delete_user(tg_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET tariff=NULL, expire_time=0, current_link=NULL, panel_client_id=NULL, uuid=NULL WHERE tg_id=?", (tg_id,))
    conn.commit()
    conn.close()
...
def get_all_active_users():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT tg_id, expire_time FROM users WHERE expire_time > ?", (int(time.time()),))
    rows = c.fetchall()
    conn.close()
    return rows
